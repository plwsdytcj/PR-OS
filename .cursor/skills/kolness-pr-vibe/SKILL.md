...
| 标签卡 | Brief 匹配与风险 | **微信式** `personal_tags` + **分类框架**（领域/身份/能力/履约/风险/商业/叙事）+ `narrative_position` |
...
| 登录态只信 Cookie 或只信 localStorage | 服务端按候选 session 逐个验证；前端登录后写 token + Cookie + 一次性 hash 兜底 |
| 达人卡片「看得见点不开」却只看 CSS | 必须登录后 smoke：达人库首卡点击后 `#creatorModal` 可见 |
